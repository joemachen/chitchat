    ...
    def on_send_message(data):
        ...
        if content.strip().lower() == "/help":
            ...
            help_lines = [
                ...
                "• `@user` or `/ping [username]` — page/mention that user",
                "• `/m` or `/msg [username] [text]` — send a direct message",
                "• `/em` or `/me [text]` — third-person emote",
                ...
                "• Right-click (or long-press) message → Reply, Add reaction, Pin/Unpin (Fam+), Edit, Delete, Hide, Mute, Report, Mark unread, Whois, Send message",
                "• Right-click user → Whois, Message, Mute, Kick (admin); on your name → Edit profile, Set status (Online/Away/DND/Invisible)",
                "• Right-click room → Move up, Move down, Mute notifications, Edit room, Unmute users (if muted)",
                ...
                "• **Spoilers** — Hide text or media with blur-to-reveal tags.",
                "  • Text: `||spoiler text goes here||`",
                "  • Links: `||https://giphy.com/funnymeme.gif||`",
                "  • Tip: Use the **Reveal all spoilers** button to unveil the whole room at once.",
                "",
                "👑 **Admin & Roles**",
                "• Super Admin sets roles (rookie, bro, fam) and permissions in Settings → Users - Roles",
                "• Save rankings for roles; Save permissions for permission checkboxes; `kick_user` = server-level kick",
                "• Destructive actions use in-app confirm dialogs",
            ]
            help_content = "\n".join(help_lines)
